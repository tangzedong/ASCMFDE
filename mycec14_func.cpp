...
double *ptM = NULL;
double *ptShift = NULL;
double *ptShuffle = NULL;
//argments f = cec14_func(x, func_num, M, shift, shuffle)
void mexFunction (int nlhs, mxArray * plhs[], int nrhs, const mxArray * prhs[])
{
    ...
    int nM = mxGetM(prhs[2]);
    int nN = mxGetN(prhs[2]);
    if (!(nM==2||nM==10||nM==30||nM==50||nM==100) || nM!=nN)
    {
        mexPrintf ("usage: f = cec14_func(x, func_num);\n");
        mexErrMsgTxt ("Error: M must be defined for D=2,10,30,50,100.");
    }
    int nShift = mxGetM(prhs[3]);
    if ( nShift < n )
    {
        mexPrintf ("usage: f = cec14_func(x, func_num);\n");
        mexErrMsgTxt ("Error: Shift must be greater than D.");
    }
    m = mxGetN (prhs[0]);
    x = mxGetPr (prhs[0]);
    ...
    f = mxGetPr (plhs[0]);
    ptM = mxGetPr(prhs[2]);
    ptShift = mxGetPr(prhs[3]);
    ptShuffle = mxGetPr(prhs[4]);
