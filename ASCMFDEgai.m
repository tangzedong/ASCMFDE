function data_MFDE = ASCMFDE(Tasks,pop,gen,selection_process,rmp,p_il,reps,index, caseid)
    ...
    while generation < gen
        ...
        for i = 1 : pop
            ...
            if urmp<=rmp
                ...
                switch caseid
                    case 2
                        newpos = F*(rand(1, dim).*(p2pos - p3pos)) * QQ{targetskill}';
                    case 3
                        newpos = F*(rand(1, dim).*(p2pos - p3pos)) * QQ{targetskill}' + F*(Pm(sourceskill2,:)- Pm(sourceskill3,:));
                    case 1
                        newpos = F*((population(p2).rnvec) - (population(p3).rnvec));
                    case 4
                        newpos = F*(p2pos * QQ{sourceskill2}' - p3pos*QQ{sourceskill2}');
                    otherwise
